    ...
    public Date getStartDate() {
        ...
        if (startDate == null) {
            ...
            if (s != null) {
                ...
                Calendar c = DateHelper.getInstance().getCalendar();
                c.setTimeInMillis(Long.parseLong(s.getValue()));
                startDate = c.getTime();
            }
        }
        if (startDate != null) {
...
//        LocalDate ywStart = new LocalDate(startDate);
        Calendar ywStart = DateHelper.getInstance().getCalendar();
        ywStart.setTime(startDate);
        ywStart.add(Calendar.DAY_OF_YEAR, 1);

        //ywStart.plusDays(dayNo - 1);
//        LocalDate dob = new LocalDate(c.getDateOfBirth());
        Calendar dob = DateHelper.getInstance().getCalendar(c.getDateOfBirth());
        // Get the age.
        int age = ywStart.get(Calendar.YEAR) - dob.get(Calendar.YEAR);
        // School year is age - 4. If the dob is before september, add 1 to year.
        int schoolYear = age - 6;
        if (dob.get(Calendar.MONTH) < Calendar.SEPTEMBER) {
            ++schoolYear;
        }
        return schoolYear;
    }
